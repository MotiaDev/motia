...
```python
from motia import ApiRequest, ApiResponse, FlowContext, api, logger
...
```python
from motia import StepConfig, EventTrigger, FlowContext, logger, step_wrapper
...
```python
from motia import StepConfig, CronTrigger, FlowContext, logger, step_wrapper
...
```python
from motia import StepConfig, ApiTrigger, EventTrigger, ApiRequest, ApiResponse, FlowContext, logger, step_wrapper
...
def handler(input, ctx: FlowContext):
    ...
    ctx.emit("order.processed", order_data)
...
def handler(data, ctx: FlowContext):
    logger.info(f"Processing high-value order: {data['amount']}")
    process_order(data)
...
```python
from motia import StepConfig, ApiTrigger, EventTrigger, FlowContext, logger, step_wrapper
...
def handler(input, ctx: FlowContext):
    logger.info(f"Processing order via {ctx.trigger.type}")
    process_order(input)
    ctx.emit("order.processed", input)
...
```python
if ctx.trigger.type == "api":
  logger.info(f"API {ctx.trigger.method} {ctx.trigger.path}")
elif ctx.trigger.type == "event":
  logger.info(f"Event {ctx.trigger.topic}")
```
...
```python
from motia import StepConfig, ApiTrigger, ApiRequest, ApiResponse, FlowContext, Stream, logger, step_wrapper
