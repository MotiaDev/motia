...
Use `iii.trigger('enqueue', { topic, data })` to publish a message. The calling function returns immediately — it does not wait for the consumer.
...
iii.registerFunction(
  ...
  async (data) => {
    ...
    logger.info('Processing payment', { orderId: id })
...
async def process_payment(data):
    ...
    logger.info("Processing payment", {"orderId": order_id})
...
iii.registerFunction(
  ...
  async (data) => {
    ...
    logger.info('Confirmation sent', { orderId: data.id })
  },
)
...
iii.register_function("orders::send-confirmation", |data| async move {
    ...
    logger.info("Confirmation sent", Some(json!({ "orderId": order_id })));
    Ok(json!(null))
});
