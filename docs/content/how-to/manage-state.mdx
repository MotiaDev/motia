...
import { registerWorker, Logger , registerWorker} from 'iii-sdk'
...
const { userId } = await iii.trigger({ function_id: 'users::create', payload: { name: 'Alice', email: 'alice@example.com' }})
...
# Then call from another function or worker
result = await iii.trigger({'function_id': 'users::create', 'payload': {'name': 'Alice', 'email': 'alice@example.com'}})
...
// Then call from another function or worker
let result = iii.trigger(TriggerRequest::new("users::create", json!({ "name": "Alice", "email": "alice@example.com" }))).await?;
...
iii.registerFunction({ id: 'users::get' }, async (input) => {
  ...
  const user = await iii.trigger({ function_id: 'state::get', payload: {
    scope: 'users',
    key: input.userId,
  }})
...
// Then call from another function or worker
const user = await iii.trigger({ function_id: 'users::get', payload: { userId: 'some-user-id' }})
...
async def get_user(input):
    ...
    user = await iii.trigger({'function_id': 'state::get', 'payload': {
        'scope': 'users',
        'key': input['userId'],
    }})
...
# Then call from another function or worker
user = await iii.trigger({'function_id': 'users::get', 'payload': {'userId': 'some-user-id'}})
...
iii.register_function("users::get", move |input| {
    ...
    async move {
        ...
        let user = iii.trigger(TriggerRequest::new("state::get", json!({
            "scope": "users",
            "key": user_id,
        }))).await?;
...
// Then call from another function or worker
let user = iii.trigger(TriggerRequest::new("users::get", json!({ "userId": "some-user-id" }))).await?;
...
// Then call from another function or worker
await iii.trigger({ function_id: 'users::record-login', payload: { userId: 'some-user-id' }})
...
iii.register_function("users::record-login", move |input| {
    ...
    async move {
        ...
        iii.trigger(TriggerRequest::new("state::update", json!({
            "scope": "users",
            "key": user_id,
            "ops": [
...
// Then call from another function or worker
iii.trigger(TriggerRequest::new("users::record-login", json!({ "userId": "some-user-id" }))).await?;
