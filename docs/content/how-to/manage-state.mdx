...
async def create_user(input):
    ...
    ctx.logger.info('User saved to state', {'userId': user_id})
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    iii.register_function("users::create", move |input| {
        ...
        async move {
            ...
            ctx.logger.info("User saved to state", Some(json!({ "userId": user_id })));
...
async def get_user(input):
    ...
    ctx.logger.info('User retrieved', {'userId': input['userId']})
    return user
...
iii.register_function("users::get", move |input| {
    ...
    async move {
        ...
        ctx.logger.info("User retrieved", Some(json!({ "userId": user_id })));
        Ok(user)
    }
});
...
async def record_login(input):
    ...
    ctx.logger.info('User login recorded', {'userId': input['userId']})
    return {'updated': True}
...
iii.register_function("users::record-login", move |input| {
    ...
    async move {
        ...
        ctx.logger.info("User login recorded", Some(json!({ "userId": user_id })));
