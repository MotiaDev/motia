...
const { messageId } = await iii.trigger({ function_id: 'chat::send', payload: { roomId: 'room-123', text: 'Hello world', author: 'alice' }})
...
def send_message(input):
    ...
    iii.trigger({'function_id': 'stream::set', 'payload': {
        'stream_name': 'chat',
        'group_id': input['roomId'],
        'item_id': message_id,
        'data': {'text': input['text'], 'author': input['author']},
...
# Then call from another function or worker
result = iii.trigger({'function_id': 'chat::send', 'payload': {'roomId': 'room-123', 'text': 'Hello world', 'author': 'alice'}})
...
// Then call from another function or worker
let result = iii.trigger(TriggerRequest::new("chat::send", json!({ "roomId": "room-123", "text": "Hello world", "author": "alice" }))).await?;
...
iii.registerFunction({ id: 'chat::list' }, async (input) => {
  ...
  const messages = await iii.trigger({ function_id: 'stream::list', payload: {
    stream_name: 'chat',
    group_id: input.roomId,
  }})
...
// Then call from another function or worker
const messages = await iii.trigger({ function_id: 'chat::list', payload: { roomId: 'room-123' }})
...
def list_messages(input):
    ...
    messages = iii.trigger({'function_id': 'stream::list', 'payload': {
        'stream_name': 'chat',
        'group_id': input['roomId'],
    }})
...
# Then call from another function or worker
messages = iii.trigger({'function_id': 'chat::list', 'payload': {'roomId': 'room-123'}})
...
iii.register_function("chat::list", move |input| {
    ...
    async move {
        ...
        let messages = iii.trigger(TriggerRequest::new("stream::list", json!({
            "stream_name": "chat",
            "group_id": room_id,
        }))).await?;
...
// Then call from another function or worker
let messages = iii.trigger(TriggerRequest::new("chat::list", json!({ "roomId": "room-123" }))).await?;
