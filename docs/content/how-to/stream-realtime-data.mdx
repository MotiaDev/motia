...
# Then call from another function or worker
result = await iii.trigger({'function_id': 'chat::send', 'payload': {'roomId': 'room-123', 'text': 'Hello world', 'author': 'alice'}})
...
async def list_messages(input):
    ...
    messages = await iii.trigger({'function_id': 'stream::list', 'payload': {
        'stream_name': 'chat',
        'group_id': input['roomId'],
    }})
...
# Then call from another function or worker
messages = await iii.trigger({'function_id': 'chat::list', 'payload': {'roomId': 'room-123'}})
