...
use iii_sdk::{get_context, III, TriggerRequest};
use serde_json::json;
...
# Trigger the task and its completion
result = iii.trigger({'function_id': 'reports::generate', 'payload': {'body': {'reportId': '123'}}})
...
```rust title="worker.rs"
    ...
    // Trigger the task and await its completion
    let result = iii.trigger(TriggerRequest::new("reports::generate", json!({ "body": { "reportId": "123" } }))).await?;
