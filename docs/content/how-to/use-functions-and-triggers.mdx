...
iii.register_function("notifications::send", HttpInvocationConfig {
    url: "https://hooks.slack.example.com/notify".to_string(),
    method: HttpMethod::Post,
    timeout_ms: Some(5000),
    headers: HashMap::from([
        ("Authorization".to_string(), format!("Bearer {}", std::env::var("SLACK_TOKEN").unwrap())),
    ]),
    auth: None,
});
...
the trigger data as the HTTP request body and treats non-2xx responses or network errors as failures.
