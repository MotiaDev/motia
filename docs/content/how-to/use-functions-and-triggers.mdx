...
use iii_sdk::{Logger, III, TriggerRequest};
use serde_json::json;
use uuid::Uuid;
...
The `registerWorker()` (Node) or `III()` / `register_worker()` (Python/Rust) call returns immediately.
...
The trade-off: if code calls `shutdown()` immediately after initialization, the connection may not yet
