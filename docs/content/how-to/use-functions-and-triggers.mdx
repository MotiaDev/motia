...
This will be addressed shortly so that a single initialization step is all that will be required in all languages.
</Callout>
...
iii = III(os.environ.get('III_URL', 'ws://localhost:49134'))
...
use iii_sdk::{Logger, TriggerRequest, III};
use serde_json::json;
use uuid::Uuid;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  iii.register_function("math::add", move |input| {
    ...
    async move {
      ...
      iii.trigger(TriggerRequest::new("state::set", json!({ "scope": "math", "key": id, "value": result }))).await?;
  ...
  // Triggerable from any other function or worker
  iii.trigger(TriggerRequest::new("math::add", json!({ "a": 2, "b": 3 }))).await?;
  Ok(())
}
```
</Tab>
...
use iii_sdk::{III, HttpAuthConfig, HttpInvocationConfig, HttpMethod, TriggerRequest};
...
iii.trigger(TriggerRequest::new("notifications::send", json!({ "channel": "#alerts", "text": "Deploy succeeded" }))).await?;
```
</Tab>
</Tabs>
...
async def calculate(data):
  result = await iii.trigger({'function_id': 'math::add', 'payload': {'a': data['a'], 'b': data['b']}})
...
iii.register_function("math::calculate", |input| async move {
  let result = iii.trigger(TriggerRequest::new("math::add", json!({"a": input["a"], "b": input["b"]}))).await?;
...
async def calculate_async(data):
  await iii.trigger({'function_id': 'math::add', 'payload': {'a': data['a'], 'b': data['b']}, 'action': TriggerAction.Void()})
...
iii.register_function("math::calculate-async", |input| async move {
  iii.trigger(
    TriggerRequest::new("math::add", json!({"a": input["a"], "b": input["b"]}))
...
Enqueue work to a named queue for async processing. The engine acknowledges with `{ messageReceiptId }` when the job is accepted. The target function receives the payload when a worker processes it. Requires queues defined in `iii-config.yaml` — see [Use Queues](/docs/how-to/use-queues).
...
iii.register_function("orders::create", move |input| {
  ...
  async move {
    ...
    let result = iii.trigger(
      TriggerRequest::new("orders::process-order", order).action(TriggerAction::enqueue("payment")),
    ).await?;
...
async def aggregation(_):
  logger = Logger()
  results = await iii.trigger({'function_id': 'state::list', 'payload': {'scope': 'math'}})
...
iii.register_function("math::aggregation", move |_| {
  ...
  async move {
    let logger = Logger();
    let results = iii.trigger(TriggerRequest::new("state::list", json!({ "scope": "math" }))).await?;
...
{/* <Callout type="info" title="Queue uses enqueue">
  ...
  See the [Queue module](/docs/modules/module-queue) for details.
</Callout> */}
...
async def double(data):
  ...
  result = await iii.trigger({'function_id': 'math::add', 'payload': {'a': data['value'], 'b': data['value']}})
...
iii.register_function("math::double", |input| async move {
  ...
  let result = iii.trigger(TriggerRequest::new("math::add", json!({"a": value, "b": value}))).await?;
...
Once a Function is registered, every other part of the system can discover and trigger it. See [Discovery](/docs/primitives-and-concepts/discovery) for how this works, including built-in functions the engine provides.
...
trigger = iii.register_trigger("http", "orders::create", {
    "api_path": "/orders",
    "http_method": "POST",
})
...
let fn_ref = iii.register_function("orders::create", |input| async move {
...
let trigger = iii.register_trigger(
    "http",
    "orders::create",
    json!({ "api_path": "/orders", "http_method": "POST" }),
)?;
...
## SDK Initialization and Registration is Synchronous

The `registerWorker()` (Node) or `III()` / `register_worker()` (Python/Rust) call returns immediately rather than returning a Promise. This is intentional.
