...
import uuid
import os
from iii import III, Logger, TriggerAction
...
| `Enqueue` | Fire-and-forget via named queue | `trigger({ function_id: 'fn', payload: data, action: TriggerAction.Enqueue({ queue: 'name' }) })` |
