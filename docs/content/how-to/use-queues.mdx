...
From any function, enqueue a job by calling `trigger()` with the `Enqueue` action and the target queue name. The enqueue operation is asynchronous — the caller does not wait for the job to be processed — but the SDK does await an acknowledgement from the engine confirming the job was accepted. The returned result contains `{ enqueued: true }` on success, or an `enqueue_error` if the queue was not found or FIFO validation failed.
...
async def create_order(input):
    ...
    await iii.trigger('orders::process-order', order,
        action=TriggerAction.Enqueue(queue='payment'))
...
async def process_order(order):
    ...
    await iii.trigger('notifications::send',
...
```python
# The 'transaction_id' field in the data is used for FIFO ordering
await iii.trigger('payments::process', {
...
# Fire-and-forget — no queue, no response
await iii.trigger('notifications::send-email',
...
| `Enqueue` | Async via named queue — returns `{ enqueued: true }` or `enqueue_error` | `trigger({ function_id: 'fn', payload: data, action: TriggerAction.Enqueue({ queue: 'name' }) })` |
...
# Enqueue to named queue (async, returns acknowledgement)
await iii.trigger('process-order', order_data,
    action=TriggerAction.Enqueue(queue='payment'))

# Void (fire-and-forget, no queue)
await iii.trigger('send-notification', data,
    action=TriggerAction.Void())
```
</Tab>
<Tab value="Rust">
...
# New: enqueue directly to the target function via a named queue
await iii.trigger('orders::process-payment', order,
    action=TriggerAction.Enqueue(queue='payment'))
