...
<Callout title="Topic-based queues" type="info">
  The topic-based API (`trigger({ function_id: 'enqueue', payload: { topic, data }, action: TriggerAction.Void() })` and `registerTrigger({ type: 'queue', ... })`) remains valid. Named Queues + Trigger Actions offer config-driven retries, concurrency, and FIFO ordering.
</Callout>
