...
| `message_group_field` | `string` | `null` | Required for FIFO — the JSON field in job data used for ordering |
...
From any function, enqueue a job by calling `trigger()` with the `Enqueue` action and the target queue name. The call is fire-and-forget — the caller returns immediately while the queue stores the job for processing.
...
import { init, Logger, TriggerActions } from 'iii-sdk'
...
iii.registerFunction({ id: 'orders::create' }, async (input) => {
  ...
  iii.trigger('orders::process-order', order, {
    action: TriggerActions.Enqueue({ queue: 'payment' }),
  })
...
import uuid
import os
from iii import III, Logger, TriggerActions
...
use iii_sdk::{Logger, TriggerAction, III};
use serde_json::json;

let iii = III::new(&std::env::var("III_BRIDGE_URL")
...
iii.register_function("orders::create", move |input| {
    ...
    async move {
        ...
        // Enqueue to the 'payment' queue
        iii.trigger_with_action(
            "orders::process-order",
            order,
            TriggerAction::enqueue("payment"),
        ).await?;
...
iii.register_function("orders::process-order", process_order)
```
</Tab>
<Tab value="Rust">
...
iii.registerFunction({ id: 'orders::process-order' }, async (order) => {
  ...
  iii.trigger('notifications::send', { orderId: order.id, type: 'payment-confirmed' }, {
    action: TriggerActions.Enqueue({ queue: 'default' }),
  })
...
async def process_order(order):
    ...
    iii.trigger('notifications::send',
        {'orderId': order['id'], 'type': 'payment-confirmed'},
        action=TriggerActions.enqueue(queue='default'))
...
iii.register_function("orders::process-order", |order| async move {
    ...
    iii.trigger_with_action(
        "notifications::send",
        json!({ "orderId": order["id"], "type": "payment-confirmed" }),
        TriggerAction::enqueue("default"),
    ).await?;
...
When order matters (e.g. payment transactions for the same account), use a FIFO queue. Set `type: fifo` and specify `message_group_field` — the field in your job data whose value determines the ordering group. Jobs with the same group value are processed strictly in order.
...
<Tab value="Node / TypeScript">
```typescript
import { TriggerActions } from 'iii-sdk'

// Fire-and-forget — no queue, no response
iii.trigger('notifications::send-email', { to: 'user@example.com', subject: 'Hello' }, {
  action: TriggerActions.Void(),
})
```
</Tab>
<Tab value="Python">
```python
from iii import TriggerActions

# Fire-and-forget — no queue, no response
iii.trigger('notifications::send-email',
...
<Tab value="Node / TypeScript">
```typescript
const { orderId } = await iii.trigger('orders::create', {
  items: ['item-1', 'item-2'],
  total: 99.99,
})
...
<Tab value="Python">
```python
result = await iii.trigger('orders::create', {
    'items': ['item-1', 'item-2'],
    'total': 99.99,
})
...
<Tab value="Rust">
```rust
let result = iii.trigger("orders::create", json!({
    "items": ["item-1", "item-2"],
    "total": 99.99,
})).await?;
...
|--------|----------|-----------|
| _(none)_ | Direct call — waits for response | `trigger('fn', data)` |
| `Enqueue` | Fire-and-forget via named queue | `trigger('fn', data, { action: TriggerActions.Enqueue({ queue: 'name' }) })` |
| `Void` | Fire-and-forget, no queue | `trigger('fn', data, { action: TriggerActions.Void() })` |
...
<Tab value="Node / TypeScript">
```typescript
import { init, TriggerActions } from 'iii-sdk'

const iii = init(process.env.III_BRIDGE_URL ?? 'ws://localhost:49134')

// Direct call (request/response)
const result = await iii.trigger('get-order', { id: '123' })

// Enqueue to named queue (fire-and-forget)
iii.trigger('process-order', orderData, {
  action: TriggerActions.Enqueue({ queue: 'payment' }),
})

// Void (fire-and-forget, no queue)
iii.trigger('send-notification', data, {
  action: TriggerActions.Void(),
})
```
</Tab>
<Tab value="Python">
```python
from iii import III, TriggerActions

iii = III(os.environ.get('III_BRIDGE_URL', 'ws://localhost:49134'))

# Direct call (request/response)
result = await iii.trigger('get-order', {'id': '123'})

# Enqueue to named queue (fire-and-forget)
iii.trigger('process-order', order_data,
    action=TriggerActions.enqueue(queue='payment'))
...
<Tab value="Rust">
```rust
use iii_sdk::{III, TriggerAction};

// Direct call (request/response)
let result = iii.trigger("get-order", json!({ "id": "123" })).await?;

// Enqueue to named queue (fire-and-forget)
iii.trigger_with_action("process-order", order_data,
    TriggerAction::enqueue("payment")).await?;

// Void (fire-and-forget, no queue)
iii.trigger_with_action("send-notification", data,
    TriggerAction::void()).await?;
```
</Tab>
</Tabs>
...
<Callout title="Deprecation notice" type="warn">
  The old topic-based API (`triggerVoid('enqueue', { topic, data })` and `registerTrigger({ type: 'queue', ... })`) still works but is deprecated. Migrate to Named Queues + Trigger Actions at your earliest convenience.
</Callout>
...
iii.trigger('orders::process-payment', order, {
  action: TriggerActions.Enqueue({ queue: 'payment' }),
})
...
# Old: enqueue via trigger_void with topic
...
<Tab value="Python">
```python title="Before (deprecated)"
iii.trigger_void('send-notification', data)
```
