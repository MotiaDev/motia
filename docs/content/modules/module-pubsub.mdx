...
async def on_order_shipped(data):
...
iii.register_function("notifications::onOrderShipped", |data| async move {
    println!("Order shipped: {:?}", data);
    Ok(json!({}))
})?;

iii.register_trigger("subscribe", "notifications::onOrderShipped", json!({
    "topic": "orders.shipped"
}))?;

iii.trigger(TriggerRequest::new("publish", json!({
    "topic": "orders.shipped",
    "data": { "orderId": "abc-123", "address": "123 Main St" }
...
async def send_email_alert(data):
    await send_email(data['userId'], f"Order {data['orderId']} shipped")
    return {}

async def send_push_alert(data):
    await send_push_notification(data['userId'], f"Order {data['orderId']} shipped")
    return {}
...
iii.register_function("notifications::sendEmailAlert", |data| async move {
...
iii.register_function("notifications::sendPushAlert", |data| async move {
    let order_id = data["orderId"].as_str().unwrap_or("");
    let user_id = data["userId"].as_str().unwrap_or("");
...
iii.register_trigger("subscribe", "notifications::sendEmailAlert", json!({
    "topic": "orders.shipped"
}))?;
iii.register_trigger("subscribe", "notifications::sendPushAlert", json!({
    "topic": "orders.shipped"
}))?;
