...
all_users = await iii.trigger({
    'function_id': 'state::list',
    'payload': {'scope': 'users'},
})
scopes = await iii.trigger({
    'function_id': 'state::list_groups',
    'payload': {},
})
```
...
let profile = iii.trigger(TriggerRequest::new("state::get", json!({
    "scope": "users",
    "key": "user-123"
}))).await?;
...
let all_users = iii.trigger(TriggerRequest::new("state::list", json!({ "scope": "users" }))).await?;
let scopes = iii.trigger(TriggerRequest::new("state::list_groups", json!({}))).await?;
```
</Tab>
</Tabs>
