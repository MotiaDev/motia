...
profile = iii.trigger('state::get', {'scope': 'users', 'key': 'user-123'})
...
all_users = iii.trigger('state::list', {'scope': 'users'})
scopes = iii.trigger('state::list_groups', {})
```
</Tab>
<Tab value="Rust">
```rust
...
let profile = iii.trigger("state::get", json!({
    "scope": "users",
    "key": "user-123"
})).await?;
...
let all_users = iii.trigger("state::list", json!({ "scope": "users" })).await?;
let scopes = iii.trigger("state::list_groups", json!({})).await?;
```
</Tab>
</Tabs>
