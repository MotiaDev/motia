...
<Callout title="How-to guidance" type="info">
...
2. **Emit events** by calling `trigger('enqueue', { topic: 'order.created', data: payload })` or `triggerVoid('enqueue', { topic, data })`. The `enqueue` function routes the payload to all subscribers of that topic.
...
|---|---|---|
| **Producer** | Calls `trigger('enqueue', { topic, data })` | Calls `trigger({ function_id, payload, action: Enqueue({ queue }) })` |
