...
Use `register_worker(...)` as the default bootstrap API. It creates an `III` client and starts the worker connection in the active Tokio runtime.
...
<ResponseField name="register_worker" type="fn(address: &str, options: InitOptions) => Result&lt;III, IIIError&gt;" required>
  Creates an `III` client and auto-starts the connection task in the current Tokio runtime. Use this as the default worker bootstrap path.
</ResponseField>
