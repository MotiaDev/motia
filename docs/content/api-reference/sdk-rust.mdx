...
<Callout type="warn" title="Deprecated methods">
  `call`, `call_with_timeout`, and `call_void` are deprecated. Use `trigger` instead. `trigger_void` is deprecated — replace with `trigger(TriggerRequest::new(fn_id, data).action(TriggerAction::void()))`.
</Callout>
