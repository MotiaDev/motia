...
async def main():
    result = await iii.trigger("math.sum", {"a": 2, "b": 3})
    print(result)  # {"sum": 5}
...
<ResponseField name="register_worker" type="function" required>
  Creates and returns a connected SDK client.
...
<ResponseField name="trigger" type="async (path: str, data: Any, timeout: float = 30.0) => Any" required>
  ...
  <AccordionGroup>
    ...
    <Accordion iconName="Code" title="Example" description="">
      ```python
      result = await iii.trigger("enqueue", {
          "topic": "user.created",
          "data": {"user_id": "123"},
      })
      ```
