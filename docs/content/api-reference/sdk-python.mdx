...
```python
from iii import III, InitOptions, ApiRequest, ApiResponse
```
...
<Callout type="warn" title="Deprecated methods">
  `call` and `call_void` are deprecated. Use `trigger` instead. `trigger_void` is deprecated — replace with `trigger({"function_id": ..., "payload": ..., "action": TriggerAction.Void()})`.
</Callout>
