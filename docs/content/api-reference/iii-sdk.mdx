...
<Callout title="Deprecated" type="warn">
  Use `trigger({ function_id, payload, action: TriggerAction.Void() })` instead. See [trigger()](#trigger) for the request-object API.
</Callout>

Triggers a function without waiting for a result (fire-and-forget). Prefer `trigger()` with `action: TriggerAction.Void()`.
...
iii.trigger({
  function_id: 'notifications::send',
  payload: { to: 'alice@example.com', message: 'Welcome!' },
  action: TriggerAction.Void(),
})
```
</Tab>
...
<ResponseField name="triggerVoid" type="deprecated" required>
  Deprecated. Use `trigger({ function_id, payload, action: TriggerAction.Void() })` instead.
</ResponseField>
...
ref = iii.register_function({'id': 'users::create'}, create_user)
```
</Tab>
<Tab value="Rust">
```rust
      ...
      <ResponseField name="trigger" type="RegisterTriggerInput" required>
        ...
        <Expandable title="RegisterTriggerInput">
          <ResponseField name="type" type="string" required>
            The trigger type (e.g., `'http'`, `'cron'`, `'queue'`). For Named Queues, use [TriggerAction.Enqueue](/docs/how-to/use-queues) instead of registering a queue trigger.
          </ResponseField>
          <ResponseField name="function_id" type="string" required>
...
|--------|-------------|------|
| `iii-sdk/stream` | Stream data types and interfaces | [Stream module](/docs/modules/module-stream) |
| `iii-sdk/state` | State management types and interfaces | [State module](/docs/modules/module-state) |
| `iii-sdk/telemetry` | OpenTelemetry utilities | [Observability module](/docs/modules/module-observability) |
