...
<Callout title="Node.js auto-connects" type="info">
  The Node SDK connects automatically when you call `registerWorker()`. Python and Rust do the same when you call `register_worker()`.
</Callout>
...
<AccordionGroup>
  <Accordion title="III" icon="network-wired">
    ...
    iii = register_worker(address="ws://127.0.0.1:49134", options=InitOptions(worker_name="worker-1"))
    ```
