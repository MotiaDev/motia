...
<Callout type="info" title="Enabled by default in Node.js">
  The Node.js SDK initializes OpenTelemetry automatically when you call `registerWorker()`. Python and Rust require an explicit `init_otel()` call.
</Callout>
...
```python
from iii import register_worker, InitOptions, OtelOptions
...
```rust
use iii_sdk::{register_worker, InitOptions, OtelOptions};
...
|---------|---------|--------|------|
| Auto-init | Yes (on `registerWorker()`) | Manual (`init_otel()`) | Manual (`init_otel()`) |
...
<Callout type="info" title="Precedence">
  Programmatic configuration passed to `registerWorker()` takes precedence over environment variables. Environment variables take precedence over defaults.
</Callout>
