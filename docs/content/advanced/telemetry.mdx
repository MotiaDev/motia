...
<Callout type="info" title="Precedence">
  Programmatic configuration passed to `init()` or `OtelConfig` takes precedence over environment variables. Environment variables take precedence over defaults.
</Callout>
