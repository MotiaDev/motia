...
async def orders_handle(data, ctx) -> dict | None:
    ...
    if isinstance(data, dict) and "path_params" in data:
        ...
        iii.trigger({
            "function_id": "state::set",
            "payload": {"scope": "orders", "key": order_id, "value": {**req.body, "id": order_id, "source": "api"}},
            "action": TriggerAction.Void(),
        })
        iii.trigger({
            "function_id": "enqueue",
            "payload": {"topic": "order.processed", "data": {"orderId": order_id, "source": "api"}},
            "action": TriggerAction.Void(),
        })
    ...
    if isinstance(data, dict) and "amount" in data:
        ...
        iii.trigger({
            "function_id": "state::set",
            "payload": {"scope": "orders", "key": order_id, "value": {**data, "id": order_id, "source": "queue"}},
            "action": TriggerAction.Void(),
        })
        iii.trigger({
            "function_id": "enqueue",
            "payload": {"topic": "order.processed", "data": {"orderId": order_id, "source": "queue"}},
            "action": TriggerAction.Void(),
        })
        return None
    ...
    pending = await iii.trigger({"function_id": "state::list", "payload": {"scope": "pending-orders"}}) or []
    for order in pending:
        iii.trigger({
            "function_id": "enqueue",
            "payload": {"topic": "order.processed", "data": {"orderId": order["id"], "source": "cron"}},
            "action": TriggerAction.Void(),
        })
...
iii.register_function("orders.handle", lambda data: orders_handle(data, Logger()))
...
iii.register_function("orders.handle", |input| async move {
    ...
    if let Ok(req) = serde_json::from_value::<ApiRequest>(input.clone()) {
        ...
        iii.trigger(TriggerRequest::new("state::set", json!({
            "scope": "orders",
            "key": order_id,
            "value": { "id": order_id, "source": "api", "amount": req.body["amount"] },
    ...
    let pending = iii.trigger(TriggerRequest::new("state::list", json!({ "scope": "pending-orders" }))).await?;
