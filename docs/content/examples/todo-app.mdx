...
def _create_todo(req: ApiRequest, ctx) -> ApiResponse:
...
```rust
use iii_sdk::{III, Streams, UpdateOp, Logger, types::ApiRequest};
use serde_json::json;
...
```python
def _update_todo(req: ApiRequest, ctx) -> ApiResponse:
    todo_id = req.path_params.get("id") if req.path_params else None

    existing = iii.trigger("stream::get", {
    ...
    updated = iii.trigger("stream::set", {
...
```python
def _delete_todo(req: ApiRequest, ctx) -> ApiResponse:
