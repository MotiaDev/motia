...
const iii = init(process.env.III_URL ?? 'ws://localhost:49134')
...
from iii import III, InitOptions, ApiRequest, ApiResponse, TriggerAction
...
iii.register_function("users.update_status", lambda data: update_user_status(
    ApiRequest(**data) if isinstance(data, dict) else data, Logger()
))
...
```rust
use iii_sdk::{III, Logger, TriggerRequest, TriggerAction, types::ApiRequest};
use serde_json::json;
...
async def get_user_status(req: ApiRequest, ctx) -> ApiResponse:
    ...
    user = await iii.trigger({"function_id": "state::get", "payload": {"scope": "users", "key": user_id}}) if user_id else None
...
async def orders_audit(_data, ctx) -> None:
    orders = await iii.trigger({"function_id": "state::list", "payload": {"scope": "orders"}}) or []
...
iii.register_function("cron.orders_audit", lambda data: orders_audit(data, Logger()))
...
|---|---|---|---|
| Write | `iii.trigger({ function_id: 'state::set', payload: { scope, key, value }, action: TriggerAction.Void() })` | `iii.trigger({'function_id': 'state::set', 'payload': {...}, 'action': TriggerAction.Void()})` | `iii.trigger(TriggerRequest::new("state::set", json!({...})).action(TriggerAction::void())).await?` |
| Read one | `await iii.trigger({ function_id: 'state::get', payload: { scope, key } })` | `await iii.trigger({'function_id': 'state::get', 'payload': {...}})` | `iii.trigger(TriggerRequest::new("state::get", json!({...}))).await?` |
| Read all in scope | `await iii.trigger({ function_id: 'state::list', payload: { scope } })` | `await iii.trigger({'function_id': 'state::list', 'payload': {...}})` | `iii.trigger(TriggerRequest::new("state::list", json!({...}))).await?` |
| Delete | `iii.trigger({ function_id: 'state::delete', payload: { scope, key }, action: TriggerAction.Void() })` | `iii.trigger({'function_id': 'state::delete', 'payload': {...}, 'action': TriggerAction.Void()})` | `iii.trigger(TriggerRequest::new("state::delete", json!({...})).action(TriggerAction::void())).await?` |
