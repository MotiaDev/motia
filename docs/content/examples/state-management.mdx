...
iii.register_function(
    RegisterFunctionMessage { id: "users.update_status".into(), description: Some("Update user status in state".into()), ..Default::default() },
    |input| async move {
...
```rust
iii.register_function(
    RegisterFunctionMessage { id: "users.get_status".into(), description: Some("Read user status from state".into()), ..Default::default() },
    |input| async move {
        let logger = Logger::new();
        let req: ApiRequest = serde_json::from_value(input)?;
        let user_id = req.path_params.get("id").cloned().unwrap_or_default();
...
```rust
iii.register_function(
    RegisterFunctionMessage { id: "cron.orders_audit".into(), description: Some("Checks for overdue orders".into()), ..Default::default() },
    |_input| async move {
        let logger = Logger::new();
