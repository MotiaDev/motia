...
def hello_api(req: ApiRequest, ctx) -> ApiResponse:
    app_name = "III App"
...
def greet_process(data: dict, ctx) -> None:
...
iii.register_function("greet.process", lambda data: greet_process(data, Logger()))
iii.register_trigger(
    type="queue",
    function_id="greet.process",
    config={"topic": "greet.process"},
...
- `iii.triggerVoid('enqueue', { topic, data })` publishes an event to the named topic. Any worker with a `queue` trigger on that topic will receive it.
- `iii.triggerVoid('state::set', { scope, key, value })` persists data to the engine's key-value store, addressed by `scope` + `key`.
