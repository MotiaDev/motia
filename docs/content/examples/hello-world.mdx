...
<Callout title="Connection is automatic" type="info">
  The Node and Python SDKs establish the WebSocket connection automatically. Keep the process alive so the worker stays registered.
</Callout>
...
```rust
// connect() then keep the event loop alive
iii.connect().await?;
loop {
