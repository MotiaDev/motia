...
- **Python**: `register_worker(address=url)` — function-based worker registration, connection is automatic
- **Rust**: `register_worker(url, ...)` — function-based worker registration, connection is automatic

| Concept | TypeScript | Python | Rust |
|---|---|---|---|
| Init | `registerWorker(url)` | `register_worker(address=url)` | `register_worker(url, ...)` |
...
| Publish event | `iii.triggerVoid('enqueue', { topic, data })` | `iii.trigger_void("enqueue", {...})` | `iii.trigger_void("enqueue", json!({...}))?` |
| State write | `iii.triggerVoid('state::set', { scope, key, value })` | `iii.trigger_void("state::set", {...})` | `iii.trigger_void("state::set", json!({...}))?` |
| Stream write | `iii.triggerVoid('stream::set', { stream_name, group_id, item_id, data })` | `iii.trigger_void("stream::set", {...})` | `streams.update(key, ops).await?` |
