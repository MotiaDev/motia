...
def orders_premium(data, ctx) -> ApiResponse:
    req = ApiRequest(**data) if isinstance(data, dict) else data
...
def orders_premium_wrapper(data):
    return orders_premium(data, Logger())
...
```rust
use iii_sdk::{III, Logger, TriggerRequest, TriggerAction};
use serde_json::json;
...
def orders_high_value(data: dict, ctx) -> None:
...
iii.register_function("orders::high_value", lambda data: orders_high_value(data, Logger()))
...
def orders_on_update(event: dict, ctx) -> None:
    logger.info("Order updated", {"scope": event.get("scope"), "key": event.get("key")})

iii.register_function("conditions::is_update", is_update)
iii.register_function("orders::on_update", lambda data: orders_on_update(data, Logger()))
