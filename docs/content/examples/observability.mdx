...
graph LR
    ...
    CreateOrder -->|"enqueue + traceId"| ProcessOrder
    ProcessOrder -->|"enqueue + traceId"| NotifyOrder
...
def create_order(req: ApiRequest, ctx) -> ApiResponse:
    body = req.body or {}
...
```python
def process_order(data: dict, ctx) -> None:
...
iii.register_function("order.process", lambda data: process_order(data, Logger()))
...
```python
def notify_order(data: dict, ctx) -> None:
    order_id = data.get("orderId")
    customer_id = data.get("customerId")
...
iii.register_function("order.notify", lambda data: notify_order(data, Logger()))
