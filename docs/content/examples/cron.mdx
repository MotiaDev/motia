...
iii.register_function("cron.periodic_job", lambda data: periodic_job(data, Logger()))
...
```python
def handle_tick(data: dict, ctx) -> None:
    logger.info("Periodic job processed", {
        "firedAt": data.get("firedAt"),
        "message": data.get("message"),
    })

iii.register_function("job.handle_tick", lambda data: handle_tick(data, Logger()))
...
def orders_sweep(_data, ctx) -> None:
...
iii.register_function("cron.orders_sweep", lambda data: orders_sweep(data, Logger()))
