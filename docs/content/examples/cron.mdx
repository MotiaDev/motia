...
graph LR
    ...
    CronFn -->|"enqueue: job.tick"| JobHandler
...
iii.register_function({"id": "cron.periodic_job"}, periodic_job)
...
iii.register_function(RegisterFunctionMessage { id: "cron.periodic_job".into(), ..Default::default() }, |_input| async move {
    let logger = Logger();
...
iii.register_trigger(RegisterTriggerInput { type_: "cron".into(), function_id: "cron.periodic_job".into(), config: json!({
    "expression": "* * * * *",
}), ..Default::default() })?;
```
...
iii.register_function({"id": "job.handle_tick"}, handle_tick)
...
iii.register_function(RegisterFunctionMessage { id: "job.handle_tick".into(), ..Default::default() }, |input| async move {
    let logger = Logger();
    logger.info("Periodic job processed", Some(input));
    Ok(json!(null))
});

iii.register_trigger(RegisterTriggerInput { type_: "queue".into(), function_id: "job.handle_tick".into(), config: json!({ "topic": "job.tick" }), ..Default::default() })?;
```
...
iii.register_function({"id": "cron.orders_sweep"}, orders_sweep)
...
iii.register_function(RegisterFunctionMessage { id: "cron.orders_sweep".into(), ..Default::default() }, |_input| async move {
    let logger = Logger();
...
iii.register_trigger(RegisterTriggerInput { type_: "cron".into(), function_id: "cron.orders_sweep".into(), config: json!({ "expression": "*/5 * * * *" }), ..Default::default() })?;
```
