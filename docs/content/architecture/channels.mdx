...
<Tab value="Rust">
```rust
use iii_sdk::{III, IIIError, ChannelReader, extract_channel_refs, ChannelDirection, TriggerRequest};
...
def coordinator_handler(input_data):
    ...
    worker_result = iii_client.trigger({
        "function_id": "stream.worker",
        "payload": {
...
use iii_sdk::{III, IIIError, ChannelReader, ChannelWriter, extract_channel_refs, ChannelDirection, TriggerRequest};
use serde_json::{json, Value};
