---
title: 'Data Channels'
description: 'Channels enable streaming binary data between functions across workers. Learn about channel creation, writer/reader pairs, and serializable refs.'
---
...
Each end also has a **ref** — a small, serializable object (`StreamChannelRef`) that can be passed as a field inside `iii.call()` data. When the receiving function deserializes the ref, the SDK automatically connects it to the engine and materializes the corresponding `ChannelWriter` or `ChannelReader`.
...
sequenceDiagram
    ...
    FnA->>Engine: iii.call("fnB", { reader: readerRef })
...
Pass a ref as a field in `iii.call()` data to hand the other end of the channel to another function. The SDK on the receiving side automatically materializes the ref into a `ChannelWriter` or `ChannelReader`.
...
|-----------|-------------------|--------|------|
| Write binary data | `writer.stream.write(data)` | `await writer.write(data)` | `writer.write(&data).await` |
| Send text message | `writer.sendMessage(msg)` | `await writer.send_message_async(msg)` | `writer.send_message(&msg).await` |
| Close | `writer.stream.end()` | `await writer.close_async()` | `writer.close().await` |
...
const sender = iii.registerFunction(
  ...
  async (input: { records: { name: string; value: number }[] }) => {
    ...
    const result = await iii.call('data.processor', {
      label: 'metrics-batch',
...
def sender_handler(input_data):
    ...
    channel.writer.close_async()

    result = iii_client.trigger("data.processor", {
        "label": "metrics-batch",
...
<Tab value="Rust">
```rust
use iii_sdk::{III, IIIError, ChannelReader, extract_channel_refs, ChannelDirection};
use serde_json::{json, Value};

let iii_for_processor = iii.clone();
iii.register_function("data.processor", move |input: Value| {
...
iii.register_function("data.sender", move |input: Value| {
    ...
    async move {
        ...
        let result = iii.call("data.processor", json!({
            "label": "metrics-batch",
...
const coordinator = iii.registerFunction(
  ...
  async (input: { text: string; chunkSize: number }) => {
    ...
    const callPromise = iii.call('stream.worker', {
...
def worker_handler(input_data):
    ...
    for chunk in reader:
        chunks.append(chunk)
        chunk_count += 1
        writer.send_message_async(json.dumps({
            "type": "progress",
...
def coordinator_handler(input_data):
    ...
    while offset < len(text_bytes):
        ...
        offset = end
    input_channel.writer.close_async()

    worker_result = iii_client.trigger("stream.worker", {
    ...
    return {"messages": messages, "binaryResult": binary_result, "workerResult": worker_result}
...
use iii_sdk::{III, IIIError, ChannelReader, ChannelWriter, extract_channel_refs, ChannelDirection};
use serde_json::{json, Value};

let iii_for_worker = iii.clone();
iii.register_function("stream.worker", move |input: Value| {
...
iii.register_function("stream.coordinator", move |input: Value| {
...
sequenceDiagram
    ...
    C->>Engine: call worker with inputChannel.readerRef + outputChannel.writerRef
