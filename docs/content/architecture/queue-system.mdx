...
The original queue model was a pub/sub system: a producer called `triggerVoid('enqueue', { topic: 'order.created', data })`, and a consumer registered with `registerTrigger({ type: 'queue', function_id: '...', config: { topic: 'order.created' } })`. This meant:
...
Before Named Queues, the SDK had three separate methods for different routing modes: `trigger()` for synchronous calls, `triggerVoid()` for fire-and-forget, and `triggerVoid('enqueue', ...)` as a special convention for queue delivery. Each was a different API surface that produced different behaviors.
...
<Callout title="Adapter behavior summary" type="info">
