...
<Tab value="Node / TypeScript">
```typescript
import { http } from 'iii'
import type { HttpRequest, HttpResponse } from 'iii'
...
```rust
use iii_sdk::{III, IIIError, ChannelWriter, extract_channel_refs, ChannelDirection};
use serde_json::{json, Value};

iii.register_function("api.example", move |input: Value| {
    let iii = iii.clone();
    async move {
...
| Write body data | `response.stream.write(data)` | `await response.writer.write(data)` | `writer.write(&data).await` |
| Close response | `response.stream.end()` / `response.close()` | `await response.writer.close_async()` | `writer.close().await` |
...
iii_client.register_trigger("http", "api.download.pdf", {
    "api_path": "download/report",
    "http_method": "GET",
})
```
...
iii.register_function("api.download.pdf", move |input: Value| {
...
iii.register_trigger("http", "api.download.pdf", json!({
    "api_path": "download/report",
    "http_method": "GET",
}))?;
```
...
iii_client.register_trigger("http", "api.events", {
    "api_path": "events",
    "http_method": "GET",
})
```
...
let iii_for_handler = iii.clone();
iii.register_function("api.events", move |input: Value| {
    let iii = iii_for_handler.clone();
...
iii.register_trigger("http", "api.events", json!({
    "api_path": "events",
    "http_method": "GET",
}))?;
```
...
iii.register_function("orders.process", move |order: Value| {
