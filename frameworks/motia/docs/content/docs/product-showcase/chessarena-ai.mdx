...
<Tabs items={['models-api', 'create-game', 'move-evaluation', 'ai-player', 'streams', 'auth']}>
  ...
  <Tab value="create-game">
    ...
    export const handler: Handlers<typeof config> = async ({ request }) => {
      ...
      const game = await createGame(request.body.players, request.user)
  ...
  <Tab value="move-evaluation">
    ...
    from streams.chess_game_move_stream import chess_game_move_stream
    ...
    async def handler(input: EvaluatePlayerMoveInput):
        ...
        try:
            ...
            move_data = await chess_game_move_stream.get(input.gameId, input.moveId)
            move_data["evaluation"] = evaluation
            await chess_game_move_stream.set(input.gameId, input.moveId, move_data)
  ...
  <Tab value="ai-player">
    ...
    import { type Handlers, type StepConfig, logger, enqueue } from 'motia'
    import { z } from 'zod'
    import { chessGameStream } from './streams/chess-game.stream'
    import { chessGameMessageStream } from './streams/chess-game-message.stream'
    ...
    export const handler: Handlers<typeof config> = async (input) => {
      const game = await chessGameStream.get('game', input.gameId)
      ...
      while (attempts < MAX_ATTEMPTS) {
        ...
        await chessGameMessageStream.set(input.gameId, messageId, {
          id: messageId,
          message: 'Thinking...',
          sender: player.ai,
          role: input.player,
        ...
        try {
          ...
          if (attempts >= MAX_ATTEMPTS) {
            ...
            await chessGameStream.set('game', game.id, {
              ...game,
              status: 'completed',
