...
export const handler: Handlers<typeof config> = async ({ request }, { state, enqueue, logger }) => {
...
async def handler(request: ApiRequest[Any], ctx: FlowContext[Any]) -> ApiResponse[Any]:
    ...
    ctx.logger.info("Order submitted", {"orderId": order_id})
...
export const handler = async ({ request }, { state, enqueue, logger }) => {
  const orderId = crypto.randomUUID()
...
async def handler(request: ApiRequest[Any], ctx: FlowContext[Any]) -> ApiResponse[Any]:
    ...
    if approved:
        ...
        ctx.logger.info("Order approved - resuming workflow", 
