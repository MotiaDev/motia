...
export function withAuth<TBody>(
  fn: (request: ApiRequest<TBody>, tokenData: TokenData, ctx: any) => Promise<ApiResponse<any, any>>
) {
  return async ({ request }: { request: ApiRequest<TBody> }, ctx: any) => {
    try {
      const tokenData = await requireAuth(request)
      return await fn(request, tokenData, ctx)
    } catch (error) {
      if (error instanceof HttpError) {
        return { status: error.status, body: { error: error.message } }
      }
...
export const handler = withAuth(async (request, tokenData, ctx) => {
