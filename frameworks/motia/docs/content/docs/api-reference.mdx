...
```typescript
export const handler: Handlers<typeof config> = async ({ request }, { traceId, logger }) => {
...
```javascript
export const handler = async ({ request }, { traceId, logger }) => {
  logger.info('Processing request', { traceId })
  return { status: 200, body: { traceId } }
}
```
...
async def handler(request: ApiRequest[Any], ctx: FlowContext[Any]) -> ApiResponse[Any]:
    ctx.logger.info("Processing request", {"trace_id": ctx.trace_id})
...
## Handler Input (MotiaHttpArgs)

HTTP handlers receive a `MotiaHttpArgs` object containing `request` and `response`. Destructure what you need.
