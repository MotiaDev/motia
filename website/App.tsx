        ...
        <div className="px-4 md:px-12 max-w-7xl mx-auto w-full flex flex-col gap-4 md:gap-8">
          <div className="flex flex-col items-start gap-4 md:gap-6 max-w-5xl">
            ...
            >
              ...
              <span className="flex items-center gap-1">
                {[0, 1, 2].map((i) => {
                  const isLit =
                    logoClickCount > i ||
                    (logoClickCount === 0 && hoverAnimIndex >= i);
                  const baseColor = bslBlink
                    ? "bg-red-500"
                    : isDarkMode
                    ? "bg-iii-accent"
                    : "bg-iii-accent-light";
                  ...
                  return (
                    ...
                    >
                      {isLit && (
                        <span
                          className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${baseColor}`}
                          style={{ animationDelay: `${i * 100}ms` }}
                        />
                      )}
                      <span
                        className={`relative inline-flex rounded-full h-1.5 w-1.5 md:h-2 md:w-2 transition-colors duration-200 ${
                          isLit ? baseColor : dimColor
                        }`}
                      />
                    </span>
                  );
              ...
              <span className="font-mono tracking-wider">
                {isGodMode ? "GOD MODE" : "KERNEL ONLINE"}
              </span>
            </button>
            ...
            <p className="text-xs md:text-base text-iii-medium max-w-xl leading-relaxed">
              ...
              >
                {" "}
                Workers self-assemble. Functions call remote GPUs like local
                imports.
              </span>{" "}
              The entire control plane in a single daemon.
            </p>

            <div className="hidden sm:flex items-center gap-3 py-2">
              <VisualArrow isDarkMode={isDarkMode} />
              <span className="text-[10px] md:text-xs text-iii-medium tracking-widest uppercase">
                infrastructure → implementation → invocation
              </span>
            </div>
