...
export const Terminal: React.FC<TerminalProps> = ({ onClose, isGodMode = false }) => {
  ...
  const addLog = (message: string, type: TerminalLog['type'] = 'info', clickableCommand?: string) => {
    setLogs(prev => [...prev, {
  ...
  useEffect(() => {
    ...
    const bootSequence = async () => {
      ...
      addLog('  ✓ RestApiModule listening on 127.0.0.1:3111', 'success');
      await new Promise(r => setTimeout(r, 150));
  ...
  const handleCommand = (cmd: string) => {
    ...
    switch (baseCommand) {
      case 'help':
        ...
        addLog('  register    - Function registration example', 'info', 'register');
        addLog('  triggers    - Trigger types (api, event, cron)', 'info', 'triggers');
        addLog('  protocol    - WebSocket protocol messages', 'info', 'protocol');
        addLog('', 'info');
        ...
        if (isGodMode) {
          ...
          addLog('  unsafe      - Unsafe operations & memory stats', 'error', 'unsafe');
          addLog('  kill-switch - BSL enforcement protocol', 'error', 'kill-switch');
        }
        break;
      ...
      case 'install':
        ...
        addLog('', 'info');
        addLog('curl -fsSL https://raw.githubusercontent.com/MotiaDev/iii-engine/main/install.sh | sh', 'success');
        addLog('', 'info');
        addLog('Install specific version:', 'warning');
        addLog('curl -fsSL [...]/install.sh | sh -s -- v0.2.1', 'info');
        addLog('', 'info');
      ...
      case 'quickstart':
        ...
        addLog('   curl -fsSL https://raw.githubusercontent.com/MotiaDev/iii-engine/main/install.sh | sh', 'info');
        addLog('', 'info');
        addLog('2. Start Redis (required for events & cron):', 'warning');
        addLog('   docker run -d -p 6379:6379 redis:alpine', 'info');
        addLog('', 'info');
        addLog('3. Run the engine:', 'warning');
        addLog('   iii --config config.yaml', 'success');
        addLog('', 'info');
      ...
      case 'status':
        ...
        addLog('Version:         v0.0.0-dev', 'warning');
        break;

      case 'version':
        addLog('═══ VERSION INFO ═══', 'system');
        addLog('', 'info');
        addLog('Engine:     v0.0.0 (development)', 'warning');
        ...
        addLog('', 'info');
        addLog('GitHub: github.com/MotiaDev/iii-engine', 'info');
        break;
      ...
      case 'ports':
        ...
        addLog('  (Workers connect here via Bridge SDK)', 'info');
        addLog('', 'info');
        ...
        addLog('  ws://127.0.0.1:31112', 'success');
        addLog('  (Real-time subscriptions)', 'info');
        addLog('', 'info');
        addLog('Redis:', 'warning');
        addLog('  redis://localhost:6379', 'success');
        addLog('  (Event, Cron, optional Streams/Logging)', 'info');
        addLog('', 'info');
      ...
      case 'sdk':
        ...
        addLog('  npm install @iii-dev/sdk', 'success');
        addLog('  pip install iii-py', 'success');
        addLog('', 'info');
      ...
      case 'workers':
        ...
        addLog('  ✓ Node.js / TypeScript (@iii-dev/sdk)', 'success');
        addLog('  ✓ Python (iii-py)', 'success');
        addLog('  ○ Rust (coming soon)', 'info');
        addLog('', 'info');
      ...
      case 'redis':
        ...
        addLog('  redis://localhost:6379', 'success');
        addLog('', 'info');
        addLog('Configure in config.yaml:', 'warning');
        addLog('', 'info');
        ...
        addLog('by disabling Event/Cron modules', 'info');
        break;
      ...
      case 'modules':
        ...
        addLog('All modules built with Rust', 'warning');
        addLog('', 'info');
        ...
        addLog('  • Stream WebSocket API', 'info');
        ...
        addLog('', 'info');
        addLog('modules::observability::LoggingModule', 'success');
        addLog('  • logger.info/warn/error functions', 'info');
        addLog('  • Redis or file-backed', 'info');
        addLog('', 'info');
        addLog('modules::shell::ExecModule', 'success');
        addLog('  • File watcher + command execution', 'info');
        addLog('  • Optional module', 'info');
        addLog('', 'info');
      ...
      case 'adapters':
        ...
        addLog('', 'info');
        addLog('StreamModule adapters:', 'warning');
        addLog('  • modules::streams::adapters::KvStore', 'success');
        ...
        addLog('', 'info');
        addLog('LoggingModule adapters:', 'warning');
        addLog('  • modules::observability::adapters::RedisLogger', 'success');
        addLog('  • modules::observability::adapters::FileLogger', 'success');
        addLog('', 'info');
        addLog('RestApiModule:', 'warning');
      ...
      case 'config':
        addLog('═══ CONFIGURATION (config.yaml) ═══', 'system');
        addLog('', 'info');
        addLog('modules:', 'success');
        addLog('  - class: modules::api::RestApiModule', 'success');
        addLog('    config:', 'success');
        ...
        addLog('      adapter:', 'success');
        addLog('        class: modules::streams::adapters::KvStore', 'success');
        addLog('        config:', 'success');
      ...
      case 'registry':
        ...
        addLog('  • Tracks: Worker ID, WebSocket channel, capabilities', 'info');
      ...
      case 'perf':
        ...
        addLog('  • REST API: 1024 concurrent requests (default)', 'info');
      ...
      case 'debug':
        ...
        addLog('  • Check worker connections: Look for "Worker connected"', 'info');
        addLog('  • Function registration: "Function registered: <path>"', 'info');
      ...
      case 'unsafe':
        ...
        addLog('  • Open sockets: 3 (WS, REST API, Streams)', 'info');
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-2 sm:p-4 ${isGodMode ? 'animate-pulse-fast' : ''}`}>
      <div className={`w-full max-w-4xl h-[calc(100vh-1rem)] sm:h-auto ${theme.bg} border ${theme.border} shadow-2xl overflow-hidden relative transition-all duration-500 rounded-lg`}>
        <div className={`flex items-center justify-between px-3 sm:px-4 py-2 ${theme.header} border-b ${theme.border}`}>
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full bg-red-500 cursor-pointer hover:bg-red-400 transition-colors touch-manipulation" onClick={onClose} />
          ...
          <span className={`text-[9px] sm:text-xs ${theme.text} font-mono uppercase tracking-wider truncate ml-2`}>
            {isGodMode ? (
              <>
                <span className="hidden sm:inline">ROOT_ACCESS // ENGINE_CORE v0.1.0</span>
                <span className="sm:hidden">ROOT_ACCESS</span>
              </>
            ) : (
              <>
                <span className="hidden sm:inline">iii_engine_debug_console v0.1.0-alpha</span>
        ...
        <div className="h-[calc(100vh-8rem)] sm:h-[24rem] md:h-[32rem] lg:h-[36rem] overflow-y-auto p-2 sm:p-3 md:p-4 font-mono text-[10px] sm:text-xs md:text-sm space-y-0.5 sm:space-y-1 bg-black">
          {logs.map((log) => (
            <div key={log.id} className="flex">
              <span className="hidden md:inline text-iii-medium/60 shrink-0 text-[10px] md:text-xs mr-2 md:mr-3">[{log.timestamp}]</span>
        ...
        <div className={`p-2 sm:p-3 md:p-4 border-t ${theme.border} bg-iii-dark/80 flex items-center gap-1.5 sm:gap-2`}>
          <span className={`${theme.caret} font-bold text-sm sm:text-base`}>{'>'}</span>
          <input
            type="text"
            value={input}
