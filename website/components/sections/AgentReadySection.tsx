import { Brain, Search, FileCode, Bell, Workflow, ArrowRight } from "lucide-react";
...
const ClaudeCodeIcon = () => (
  <svg viewBox="25 25 50 50" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor">
...
const CursorIcon = () => (
  <svg viewBox="30 28 40 42" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor">
...
const GeminiIcon = () => (
  <svg viewBox="28 28 44 44" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor">
...
const WindsurfIcon = () => (
  <svg viewBox="22 30 56 40" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor">
...
const TraeIcon = () => (
  <svg viewBox="30 35 40 32" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor">
    <path clipRule="evenodd" d="m67 63.1005h-29.1437v-4.8535h-4.8563v-19.4296h34zm-29.1437-4.8535h24.2874v-14.5746h-24.2874zm12.1451-7.361-3.4354 3.434-3.434-3.434 3.434-3.434zm9.7141-.0014-3.434 3.4326-3.4354-3.4326 3.4354-3.4354z" fillRule="evenodd"/>
  </svg>
);
...
const RooIcon = () => (
  <svg viewBox="25 30 50 40" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor">
...
const CopilotIcon = () => (
  <svg viewBox="-3 -3 30 30" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor">
...
const ClineIcon = () => (
  <svg viewBox="-3 -3 30 30" className="w-8 h-8 md:w-10 md:h-10" fill="currentColor" fillRule="evenodd">
...
export function AgentReadySection({ isDarkMode = true }: AgentReadySectionProps) {
  ...
  const cardHover = isDarkMode ? "hover:border-iii-info/40" : "hover:border-[#0891b2]/40";
      ...
      <div className="relative z-10">
        <div className="text-center mb-10 md:mb-16 space-y-4">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className={`text-xs md:text-sm font-mono tracking-wider uppercase ${infoColor}`}>
              Agent-Ready
            </span>
          </div>
          ...
          <p className={`text-sm md:text-base lg:text-lg max-w-3xl mx-auto ${textSecondary}`}>
        ...
        <div className="mb-10 md:mb-14 overflow-hidden">
          <p className={`text-center text-xs uppercase tracking-widest mb-6 ${textSecondary}`}>
            The universal agentic layer
          </p>
          <div
            className="flex w-max"
            style={{ animation: 'agent-scroll 40s linear infinite' }}
          >
            {[...agentLogos, ...agentLogos].map((logo, i) => {
              const Icon = logo.icon;
              return (
                <div key={i} className="flex flex-col items-center gap-2 px-4 md:px-6 shrink-0">
                  <div className={iconBase}>
                    <Icon />
                  </div>
                  <span className={`text-[10px] whitespace-nowrap ${textPrimary}`}>{logo.name}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-4">
          ...
          <div className={`col-span-1 md:col-span-4 lg:col-span-4 group relative overflow-hidden rounded-xl border transition-all duration-300 p-6 md:p-8 ${cardBg} ${cardBorder} ${cardHover}`}>
            <div className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -mr-16 -mt-16 transition-opacity opacity-0 group-hover:opacity-100 ${isDarkMode ? 'bg-iii-info/5' : 'bg-[#0891b2]/5'}`} />
            <div className="relative z-10 flex flex-col gap-4 md:gap-6">
              <div className={`p-2 border rounded-lg w-fit ${iconBg} ${cardBorder}`}>
                <Search className={`w-5 h-5 md:w-7 md:h-7 ${infoColor}`} />
              </div>
              <div className="space-y-3">
                <h3 className={`text-xl md:text-2xl font-semibold tracking-tight ${textPrimary}`}>Discover & Invoke</h3>
                <p className={`text-xs md:text-sm leading-relaxed max-w-lg ${textSecondary} transition-colors ${isDarkMode ? 'group-hover:text-gray-300' : 'group-hover:text-gray-600'}`}>
                  Agents find and execute functions across the system without pre-configuration
                </p>
              </div>
              <div className="flex items-center gap-2 md:gap-3 mt-2">
                {['Agent', 'Discover', 'Invoke'].map((label, i) => (
                  <div key={label} className="flex items-center gap-2 md:gap-3">
                    <span className={`px-3 py-1.5 rounded-lg border text-xs font-mono ${iconBg} ${cardBorder} ${textPrimary}`}>
                      {label}
                    </span>
                    {i < 2 && <ArrowRight className={`w-3.5 h-3.5 ${infoColor}`} />}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className={`col-span-1 md:col-span-2 lg:col-span-2 group relative overflow-hidden rounded-xl border transition-all duration-300 p-4 md:p-6 ${cardBg} ${cardBorder} ${cardHover}`}>
            <div className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -mr-16 -mt-16 transition-opacity opacity-0 group-hover:opacity-100 ${isDarkMode ? 'bg-iii-info/5' : 'bg-[#0891b2]/5'}`} />
            <div className="relative z-10 flex flex-col gap-3">
              <div className={`p-2 border rounded-lg w-fit ${iconBg} ${cardBorder}`}>
                <FileCode className={`w-4 h-4 md:w-5 md:h-5 ${infoColor}`} />
              </div>
              <h3 className={`text-base md:text-lg font-semibold ${textPrimary}`}>Self-Describing Schemas</h3>
              <p className={`text-[10px] md:text-xs leading-relaxed ${textSecondary}`}>
                Functions include input/output schemas for autonomous payload construction
              </p>
            </div>
          </div>
          ...
          <div className={`col-span-1 md:col-span-2 lg:col-span-2 group relative overflow-hidden rounded-xl border transition-all duration-300 p-4 md:p-6 ${cardBg} ${cardBorder} ${cardHover}`}>
            ...
            <div className="relative z-10 flex flex-col gap-3">
              ...
              <h3 className={`text-base md:text-lg font-semibold ${textPrimary}`}>Real-Time Updates</h3>
              <p className={`text-[10px] md:text-xs leading-relaxed ${textSecondary}`}>
                Capabilities change — agents adapt instantly, no polling
              </p>
            </div>
          </div>
          ...
          <div className={`col-span-1 md:col-span-2 lg:col-span-2 group relative overflow-hidden rounded-xl border transition-all duration-300 p-4 md:p-6 ${cardBg} ${cardBorder} ${cardHover}`}>
            ...
            <div className="relative z-10 flex flex-col gap-3">
              ...
              <h3 className={`text-base md:text-lg font-semibold ${textPrimary}`}>Compose Workflows</h3>
              <p className={`text-[10px] md:text-xs leading-relaxed ${textSecondary}`}>
          ...
          <div className={`col-span-1 md:col-span-2 lg:col-span-2 group relative overflow-hidden rounded-xl border transition-all duration-300 p-4 md:p-6 ${cardBg} ${cardBorder} ${cardHover}`}>
            ...
            <div className="relative z-10 flex flex-col gap-3">
              ...
              <h3 className={`text-base md:text-lg font-semibold ${textPrimary}`}>Register Capabilities</h3>
              <p className={`text-[10px] md:text-xs leading-relaxed ${textSecondary}`}>
                Agents become active participants, not passive consumers
              </p>
            </div>
          </div>
