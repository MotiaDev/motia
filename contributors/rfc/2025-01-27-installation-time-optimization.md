...
### 1. Redis Memory Server Compilation Deferral

**Problem**: Redis binaries are compiled during `npm install` even when users never run the server. This adds 30-60 seconds to installation time.

**Solution**: Defer Redis compilation to first run using environment variable configuration.

**Implementation**:

Add to `.npmrc` generated by `npx motia create`:
```
redisms_download_on_start=true
```

This tells `redis-memory-server` to defer download/compilation until the first time Redis is actually needed (when running `motia dev` or `motia start`).

**Alternative - Dynamic Import**:

For better user experience without configuration, move to `optionalDependencies` and use dynamic import:
...
**Additional Options**:
- **External Redis**: Support `MOTIA_REDIS_URL` for users with existing Redis
- **System Binary**: Support `REDISMS_SYSTEM_BINARY` for pre-installed Redis
