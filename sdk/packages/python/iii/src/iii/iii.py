...
class III:
    ...
    def register_trigger_type(
        ...
        Args:
            trigger_type: Trigger type input with ``id`` and ``description``, or a dict.
    ...
    def unregister_trigger_type(self, trigger_type: RegisterTriggerTypeInput | dict[str, Any]) -> None:
        ...
        Args:
            trigger_type: The trigger type input to unregister, or a dict with ``id``.
        """
        if isinstance(trigger_type, dict):
            trigger_type = RegisterTriggerTypeInput(**trigger_type)
    ...
    def register_trigger(self, trigger: RegisterTriggerInput | dict[str, Any]) -> Trigger:
        ...
        Args:
            trigger: Trigger input with ``type``, ``function_id``, and ``config``, or a dict.

        Returns:
            A Trigger handle with an ``unregister()`` method.

        Examples:
            >>> trigger = iii.register_trigger({'type': 'http', 'function_id': 'greet', 'config': {'api_path': '/greet'}})
            >>> trigger.unregister()
        """
    ...
    def register_function(
        ...
        Args:
            func: Function input with ``id``, ``description``, etc., or a dict.
    ...
    def trigger(self, request: "dict[str, Any] | TriggerRequest") -> Any:
        """Invoke a remote function (synchronous wrapper).
