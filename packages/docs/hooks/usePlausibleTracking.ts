...
export function usePlausibleTracking() {
  ...
  const trackDownload = (filename: string, method = 'click') => {
  ...
  const trackPurchase = (amount: number, currency = 'USD', productTitle?: string) => {
    trackEvent('Purchase', productTitle ? { productTitle } : undefined, {
      amount,
      currency,
    })
  }
