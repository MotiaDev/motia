...
## Organizing Steps with Flows

Steps can be grouped together using the `flows` property. This helps you organize related Steps into logical workflows and visualize them better in Workbench.

### Why Use Flows?

Think of flows as tags for your Steps. They help you:

- **Group related Steps** → All Steps working together on the same feature
- **Visualize workflows** → See connected Steps together in Workbench
- **Organize your code** → Find Steps that belong to the same business flow
- **Share context** → Document which Steps are part of which workflow

A Step can belong to multiple flows. Just add more flow names to the array.
...
export const handler: Handlers['CreateOrder'] = async (req, { emit }) => {
  await emit({ topic: 'order.created', data: req.body });
  return { status: 200, body: { ok: true } };
};
```
...
export const handler: Handlers['ProcessOrder'] = async (input, { emit }) => {
  // Process the order
  await emit({ topic: 'order.processed', data: input });
};
```
...
async def handler(req, ctx):
    await ctx.emit({"topic": "order.created", "data": req.body})
    return {"status": 200, "body": {"ok": True}}
```
...
async def handler(input, ctx):
    # Process the order
    await ctx.emit({"topic": "order.processed", "data": input})
```
...
const handler = async (input, { emit }) => {
  // Process the order
  await emit({ topic: 'order.processed', data: input });
};
...
👉 Both Steps are now part of the `order-management` flow. In Workbench, they'll appear grouped together.

### Multiple Flows

A Step can belong to multiple flows at once:
...
export const config: EventConfig = {
  ...
  type: 'event',
  subscribes: ['order.processed', 'user.created'],
  flows: ['order-management', 'user-onboarding'],
...
👉 This Step will show up in both the `order-management` and `user-onboarding` flows in Workbench.

### Flows are Optional

You don't have to use flows. They're just for organization and visualization. Steps work the same with or without them:

<Tabs items={['TypeScript', 'Python', 'JavaScript']}>
<Tab value='TypeScript'>

```ts
// This works fine - no flows needed
export const config: ApiRouteConfig = {
  name: 'HealthCheck',
  type: 'api',
  path: '/health',
...
**Use flows when:**
- You have multiple Steps working together
- You want to visualize how Steps connect in Workbench
- You're building multiple features and want to keep them organized
- You're sharing your project with others and want clear documentation

**Skip flows when:**
- You have simple, standalone Steps
- You're just prototyping
- Organization isn't important for your use case
