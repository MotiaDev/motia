...
| `messageGroupId` | `string` | - | `null` | Field name to extract from event data for FIFO grouping, or `traceId` for trace-based grouping. Can be overridden by passing `messageGroupId` explicitly in emit options. |
...
When emitting events to FIFO queues, you can pass an explicit `messageGroupId` as part of the emit options. This overrides any automatic extraction from the event data:
...
**When to use explicit `messageGroupId` in emit:**

- **Override automatic extraction**: When you need different grouping logic than what's in your event data
- **Dynamic grouping**: When the group ID needs to be computed or derived from multiple fields
- **Cross-step consistency**: When emitting from API or Cron steps to FIFO event steps

**Priority order for FIFO message grouping:**

1. Explicit `messageGroupId` passed in `emit()` options (highest priority)
2. Automatic extraction from event data using `queue.messageGroupId` field name
3. If neither is provided for a FIFO queue, the message will fail validation
...
- **FIFO Queue Rule**: When `queue.type` is `fifo`, `messageGroupId` must either be specified in the queue config OR passed explicitly when calling `emit()`
- **messageGroupId Reference**: When specified in queue config, must reference a valid field in your input schema (or use `traceId` for trace-based grouping)
