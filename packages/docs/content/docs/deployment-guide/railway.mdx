...
// node-redis format (State, Streams, Cron)
...
<Callout type="info">
**Why two config formats?** The Redis adapters use different client libraries. State, Streams, and Cron use `node-redis` (v4), while BullMQ uses `ioredis`. Each expects a slightly different config structure.
</Callout>
