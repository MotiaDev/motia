...
function getRedisConfig() {
  ...
  // Parse REDIS_URL if provided (works with Docker, Railway, Fly.io, etc.)
  if (redisUrl) {
    try {
      ...
      return {
        ...
        useTls: url.protocol === 'rediss:', // rediss:// means TLS
      }
    } catch (e) {
...
// USE_REDIS takes priority, otherwise check NODE_ENV
const useRedis = process.env.USE_REDIS === 'true' || 
  (process.env.USE_REDIS !== 'false' && process.env.NODE_ENV === 'production')

// node-redis v4 format (State, Streams, Cron adapters)
const nodeRedisConfig = {
  socket: {
    host: redis.host,
    port: redis.port,
    ...(redis.useTls && { tls: true }),
  },
  ...(redis.password && { password: redis.password }),
}

// ioredis format (BullMQ adapter)
const ioRedisConfig = {
  host: redis.host,
  port: redis.port,
  ...(redis.password && { password: redis.password }),
...
export default defineConfig({
  ...
  } : undefined, // Falls back to in-memory adapters when not using Redis
})
```

<Callout type="warn">
**Important:** The Redis adapters use two different client libraries:
- `node-redis` (v4) for State, Streams, and Cron - uses `socket: { host, port }` format
- `ioredis` for BullMQ - uses `host, port` directly

Make sure you use the correct format for each!
</Callout>
