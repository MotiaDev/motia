...
**Error:** `make: not found` or `gcc: not found` during installation or runtime.

**Cause:** The `redis-memory-server` package requires build tools (`make`, `gcc`, etc.) to compile Redis from source. These are not included in minimal images.
