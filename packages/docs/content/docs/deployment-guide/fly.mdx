...
function getRedisConfig() {
  // Fly sets REDIS_URL when you attach Upstash Redis
  ...
  if (redisUrl) {
    try {
      ...
      return {
        ...
        useTls: url.protocol === 'rediss:', // Upstash uses TLS
      }
    } catch (e) {
...
<Callout type="info">
**Upstash uses TLS.** The config above automatically enables TLS when it detects `rediss://` in the URL.
</Callout>
