...
**Parameters:**
- `req` - Dictionary with `body`, `headers`, `pathParams`, `queryParams`, `rawBody`
...
export const handler: Handlers['GetUser'] = async (req, ctx) => {
  ...
  // Raw body (unparsed request body as string)
...
- `rawBody` - The raw unparsed request body as a string (same body that was sent but without type enforcement/parsing)
...
npx motia create --template motia-tutorial-python my-python-app
```
...
|----------|-------------|----------|
| `starter-multilang` | Starter (All languages; TS/JS + Python) | Polyglot project with TypeScript API and Python event processing |
