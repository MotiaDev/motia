...
- **Files**: 
  - `src/gmail-get-auth-url.step.ts`: Generates OAuth2 authorization URL
  - `src/gmail-auth.step.ts`: Handles authorization code exchange
  - `src/gmail-token-status.step.ts`: Checks token validity and refreshes if needed

### 2. Gmail Webhook (API Step)
- **File**: `src/gmail-webhook.step.ts`
...
- **File**: `src/analyze-email.step.py`
...
- **File**: `src/organize-email.step.ts`
...
- **File**: `src/auto-responder.step.ts`
...
- **File**: `src/daily-summary.step.ts`
...
- `src/` - Contains all workflow steps
