...
A Step is just a file with a `config` and a `handler`. Motia auto-discovers these files (from `/src` or `/steps`) and connects them automatically.
...
Learn more about Steps here: [What is a Step?](/docs/concepts/steps/defining-steps).
