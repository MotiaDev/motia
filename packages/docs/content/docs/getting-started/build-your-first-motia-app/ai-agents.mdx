...
Keep it positive, realistic, and adoption-focused.`
        ...
        try {
          ...
          if (logger) {
            logger.warn('⚠️ AI response parsing failed, using fallback profile', { petId })
          }
        }
...
The real power comes when your AI agents integrate with a workflow orchestrator. The orchestrator subscribes to the events emitted by AI agents and handles the actual state transitions.

The orchestrator configuration shows it subscribes to AI agent events:
  ...
  <Tab value="TypeScript">
    ...
    export const config = {
      ...
      subscribes: [
        ...
        'ts.health.treatment_required',       // From Health Review Agent
        'ts.health.no_treatment_needed',      // From Health Review Agent
        'ts.adoption.needs_data',             // From Adoption Review Agent
        'ts.adoption.ready'                   // From Adoption Review Agent
      ],
      emits: [
    ...
    // The orchestrator has transition rules that handle AI agent events
  ...
  <Tab value="Python">
    ...
    config = {
        ...
        "subscribes": [
            ...
            "py.health.treatment_required",       # From Health Review Agent
            "py.health.no_treatment_needed",      # From Health Review Agent
            "py.adoption.needs_data",             # From Adoption Review Agent
            "py.adoption.ready"                   # From Adoption Review Agent
        ],
        "emits": [
    ...
    # Transition rules that handle AI agent events
    TRANSITION_RULES = [
        # ... other rules ...
  ...
  <Tab value="JavaScript">
    ...
    exports.config = {
      ...
      subscribes: [
        ...
        'js.health.treatment_required',       // From Health Review Agent
        'js.health.no_treatment_needed',      // From Health Review Agent
        'js.adoption.needs_data',             // From Adoption Review Agent
        'js.adoption.ready'                   // From Adoption Review Agent
      ],
      emits: [
...
Be specific about what you want from the AI. See the actual prompts in `agent-decision-framework.ts/js`.

### 3. Implement Fallbacks

Always have a plan B if the AI fails (see `ai-profile-enrichment.step.ts` for fallback profile example).

### 4. Use Idempotency Checking

Prevent duplicate AI calls by checking recent artifacts:
...
if (recentArtifacts.length > 0) {
  return recentArtifacts[recentArtifacts.length - 1]
}
```
...
Never trust AI output blindly - validate the chosen emit exists in your registry:
...
Your pet shelter now has intelligent AI agents making decisions! But how do you give users real-time feedback while all this AI processing happens in the background?
...
- **AI Agent Streaming** - Stream AI enrichment progress in real-time
...
<Cards>
  <Card href="/docs/getting-started/build-your-first-motia-app/streaming" title="Next: Real-Time Streaming">
    Add live updates using streams in your handlers
  </Card>
  <Card href="/docs/concepts/ai-agents" title="AI Agents Deep Dive">
    Learn more about building intelligent agents with Motia
  </Card>
</Cards>
...
- Build multi-modal agents (image analysis for pet photos)
