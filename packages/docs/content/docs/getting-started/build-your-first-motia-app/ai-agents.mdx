...
AI agents transform your workflows from simple automation into intelligent systems that make decisions. Instead of hardcoding every rule, you let AI analyze context and choose the right action.

There are two types of AI agents in this system:

**Content Generation Agents** - They enrich your data automatically without making workflow decisions. Think of them as helpful assistants that fill in details.

**Decision-Making Agents** - These agents evaluate situations and choose which path the workflow should take. They're like smart routers that understand context.

In our pet shelter, AI helps with:
- Writing engaging pet profiles for potential adopters
- Assessing health conditions from symptoms
- Determining if pets have complete adoption data
- Making judgment calls that would normally require staff review

The key difference from traditional code? Instead of writing complex if-else logic, you describe what you want the AI to consider, and it makes informed decisions based on that context.
...
Explore more examples in the [Motia Examples Repository](https://github.com/MotiaDev/motia-examples) or dive deeper into [AI integration patterns](https://www.motia.dev/docs/concepts/ai-agents).
