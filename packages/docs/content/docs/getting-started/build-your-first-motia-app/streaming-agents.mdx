...
- **AI Agent Streaming** - AI enrichment with live progress updates
...
You've built APIs that return immediately, background jobs that process asynchronously, workflows that orchestrate complex logic, and AI agents that make intelligent decisions. But how do you give users real-time feedback while all this async processing happens in the background?
...
- AI agents stream enrichment progress (bio generation, breed analysis)
...
<Tabs items={['TypeScript', 'Python', 'JavaScript']}>
  <Tab value="TypeScript">
    ...
    export const handler: Handlers['TsCreatePet'] = async (req, { emit, logger, streams, traceId }) => {
      try {
        ...
        logger.info('🐾 Pet created', { petId: pet.id, name: pet.name, species: pet.species })

        // Initialize stream and return the result
        ...
        // Emit events to trigger background workflows
        if (emit) {
          await emit({
  ...
  <Tab value="Python">
    ...
    async def handler(req, ctx=None):
        ...
        if logger:
            logger.info("🐾 Pet created", {"petId": pet["id"], "name": name, "species": species})

        # Initialize stream
        result = await streams.petCreation.set(trace_id, 'message', { 
            "message": f"Pet {name} (ID: {pet['id']}) created successfully - Species: {species}, Age: {age_val} months, Status: {pet['status']}"
        })
        ...
        if emit:
            await emit({
                ...
                "data": {"petId": pet["id"], "event": "pet.created", "name": name, "species": species, "traceId": trace_id}
            })
  ...
  <Tab value="JavaScript">
    ...
    const handler = async (req, { emit, logger, streams, traceId }) => {
      const b = req.body || {}
      ...
      if (logger) {
        logger.info('🐾 Pet created', { petId: pet.id, name: pet.name })
      }

      // Initialize stream
      const result = await streams.petCreation.set(traceId, 'message', {
        message: `Pet ${name} (ID: ${pet.id}) created successfully - Species: ${b.species}, Status: new`
      })
      ...
      if (emit) {
        await emit({
          ...
          data: { petId: pet.id, name, species: b.species, traceId }
        })
...
<Tabs items={['TypeScript', 'Python', 'JavaScript']}>
  <Tab value="TypeScript">
    ...
    export const config = {
      ...
      description: 'Background job that sets feeding reminders with stream updates',
    ...
    export const handler = async (input: any, context?: any) => {
      ...
      try {
        // Calculate next feeding time (2 hours from now)
        const nextFeedingAt = Date.now() + (2 * 60 * 60 * 1000)
        
        // Update pet with feeding schedule
        const updates = {
          notes: 'Welcome to our shelter! Initial health screening scheduled.',
        ...
        if (logger) {
          logger.info('✅ Feeding reminder set', { 
            petId, 
            nextFeedingAt: new Date(nextFeedingAt).toISOString()
          })
        }

        // Push real-time stream updates
  ...
  <Tab value="Python">
    ...
    async def handler(input_data, ctx=None):
        ...
        if logger:
            logger.info("📋 Setting next feeding reminder", {"petId": pet_id})

        try:
            # Calculate next feeding time
            next_feeding_at = int(time.time() * 1000) + (2 * 60 * 60 * 1000)
            ...
            if not updated_pet:
                if logger:
                    logger.error("❌ Pet not found", {"petId": pet_id})
                return

            if logger:
                logger.info("✅ Feeding reminder set", {"petId": pet_id})

            # Push stream updates
            if streams and hasattr(streams, 'petCreation') and trace_id:
                await streams.petCreation.set(trace_id, 'message', {
                    "message": f"Pet {updated_pet['name']} entered quarantine period"
                })

                # Check symptoms
                symptoms = updated_pet.get("symptoms", [])
                if not symptoms:
                    ...
                    await asyncio.sleep(1)
                    await streams.petCreation.set(trace_id, 'message', {
                        "message": f"Health check failed for {updated_pet['name']} - symptoms: {', '.join(symptoms)}"
                    })
            ...
            if logger:
                logger.error("❌ Job error", {"petId": pet_id, "error": str(error)})
    ```
  </Tab>
  <Tab value="JavaScript">
    ...
    const handler = async (input, { emit, logger, streams, traceId }) => {
      const { petId, enqueuedAt } = input

      if (logger) {
        logger.info('📋 Setting next feeding reminder', { petId })
      }
      ...
      try {
        ...
        if (logger) {
          logger.info('✅ Feeding reminder set', { petId })
        }
        ...
        if (streams?.petCreation && traceId) {
          ...
          if (!updatedPet.symptoms || updatedPet.symptoms.length === 0) {
            ...
            await delay(1000)
            await streams.petCreation.set(traceId, 'message', {
              message: `Health check failed for ${updatedPet.name} - symptoms: ${updatedPet.symptoms.join(', ')}`
            })
        ...
        if (logger) {
          logger.error('❌ Job error', { petId, error: error.message })
        }
      }
    }
...
## Step 4: AI Agent Streaming

AI agents can also stream progress updates as they generate content. This provides live feedback during potentially long-running AI operations.
...
<Tabs items={['TypeScript', 'Python', 'JavaScript']}>
  <Tab value="TypeScript">
    ...
    export const config = {
      ...
      description: 'AI agent that enriches profiles with streaming updates',
    ...
    export const handler = async (input: any, context?: any) => {
      ...
      if (streams?.petCreation && traceId) {
        await streams.petCreation.set(traceId, 'message', {
          message: `Starting AI profile enrichment for ${name}...`
        })
      }
      ...
      if (!apiKey) {
        if (logger) {
          logger.warn('⚠️ OpenAI API key not found', { petId })
        }
        return
      }
      ...
      try {
        ...
        // Stream progress updates as we generate different fields
        const fields = ['bio', 'breedGuess', 'temperamentTags', 'adopterHints']
        
        for (const field of fields) {
          if (streams?.petCreation && traceId) {
            await streams.petCreation.set(traceId, 'message', {
              message: `Generating ${field} for ${name}...`
            })
          }
          await new Promise(resolve => setTimeout(resolve, 500))
        }

        // Call OpenAI (simplified for example)
        const prompt = `Generate a pet profile for ${name}, a ${species}.`
        
        // ... OpenAI API call here ...
        
        const aiProfile = {
          bio: `Meet ${name}, a wonderful ${species}!`,
          breedGuess: 'Mixed breed',
          temperamentTags: ['friendly', 'playful'],
          adopterHints: 'Great for families!'
        }

        // Update pet with enriched data
        TSStore.update(petId, aiProfile)

        // Stream completion
        if (streams?.petCreation && traceId) {
          await streams.petCreation.set(traceId, 'message', {
            message: `✅ AI profile enrichment completed for ${name}!`
          })
        }

        if (logger) {
          logger.info('✅ AI Profile Enrichment completed', { petId })
        }

      } catch (error: any) {
        if (logger) {
          logger.error('❌ AI enrichment error', { petId, error: error.message })
        }
        
        if (streams?.petCreation && traceId) {
          await streams.petCreation.set(traceId, 'message', {
            message: `❌ AI enrichment failed for ${name}, using fallback profile`
          })
        }
      }
    }
  ...
  <Tab value="Python">
    ...
    async def handler(input_data, ctx=None):
        ...
        if streams and hasattr(streams, 'petCreation') and trace_id:
            await streams.petCreation.set(trace_id, 'message', {
                "message": f"Starting AI profile enrichment for {name}..."
            })

        api_key = os.environ.get("OPENAI_API_KEY")
        if not api_key:
            if logger:
                logger.warn("⚠️ OpenAI API key not found", {"petId": pet_id})
            return
        ...
        try:
            ...
            fields = ['bio', 'breedGuess', 'temperamentTags', 'adopterHints']
            
            for field in fields:
                if streams and hasattr(streams, 'petCreation') and trace_id:
                    await streams.petCreation.set(trace_id, 'message', {
                        "message": f"Generating {field} for {name}..."
                    })
                await asyncio.sleep(0.5)

            # AI generation (simplified)
            ai_profile = {
                "bio": f"Meet {name}, a wonderful {species}!",
                "breedGuess": "Mixed breed",
                "temperamentTags": ["friendly", "playful"],
                "adopterHints": "Great for families!"
            }

            pet_store.update(pet_id, ai_profile)

            # Stream completion
            if streams and hasattr(streams, 'petCreation') and trace_id:
                await streams.petCreation.set(trace_id, 'message', {
                    "message": f"✅ AI profile enrichment completed for {name}!"
                })

            if logger:
                logger.info("✅ AI Profile Enrichment completed", {"petId": pet_id})

        except Exception as error:
            if logger:
                logger.error("❌ AI enrichment error", {"petId": pet_id, "error": str(error)})
            
            if streams and hasattr(streams, 'petCreation') and trace_id:
                await streams.petCreation.set(trace_id, 'message', {
                    "message": f"❌ AI enrichment failed for {name}, using fallback"
                })
    ```
  </Tab>
  <Tab value="JavaScript">
    ...
    const handler = async (input, { logger, streams, traceId }) => {
      const { petId, name, species } = input
      ...
      try {
        ...
        if (streams?.petCreation && traceId) {
          await streams.petCreation.set(traceId, 'message', {
            message: `❌ AI enrichment failed for ${name}, using fallback`
          })
        }
      }
    }
...
### How AI Agent Streaming Works

AI agents stream progress as they work:
...
### Monitor Stream Updates in Workbench
...
### Test 2: Create Pet with Symptoms
...
![stream-updates](../../img/build-your-first-app/streams-symptoms-output.png)
...
![stream-updates](../../img/build-your-first-app/streams-healthy-output.png)
...
- **AI Agents** - Make intelligent decisions with AI
