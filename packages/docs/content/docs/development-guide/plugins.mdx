...
<Steps>
  <Step title="Generate a plugin project">
    ...
    ```bash
    pnpm dlx motia create my-plugin --template plugin
    ```
  </Step>
  <Step title="Use interactive mode (optional)">
    Prefer to pick from templates interactively? Launch the wizard:

    ```bash
    pnpm dlx motia create
