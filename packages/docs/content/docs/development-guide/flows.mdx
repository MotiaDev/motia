...
- **Real connections** - When a Step `emits` a topic another Step `subscribes` to
- **Virtual connections** - Use `virtualEmits` and `virtualSubscribes` for visualization only
...
![Virtual connections in Workbench](../img/virtual-emit-subscribe.png)
