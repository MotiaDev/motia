...
```python title="src/create_order_step.py"
import uuid
...
👉 **Notice:** We save the order right away with `currentStep` and `completedSteps`. This tracks where we are.
...
```python title="src/process_payment_step.py"
import time
...
async def handler(input_data, context):
    ...
    if not order:
        context.logger.error("Order not found", {"orderId": order_id})
        return
    
    # Skip if we already did this (makes it safe to retry)
    if "payment" in order["completed_steps"]:
        context.logger.info("Payment already done", {"orderId": order_id})
        return
    ...
    try:
        ...
        context.logger.info("Payment completed", {"orderId": order_id, "paymentId": payment_id})
        ...
        await context.state.set("orders", order_id, order)
        context.logger.error("Payment failed", {"orderId": order_id, "error": str(e)})
```
...
```python title="src/shipping_webhook_step.py"
from datetime import datetime
...
async def handler(req, context):
    ...
    if order["current_step"] != "awaiting_shipment":
        context.logger.warn("Order not ready for shipping", {
            "orderId": order_id,
    ...
    context.logger.info("Order shipped", {"orderId": order_id, "trackingNumber": tracking_number})
...
👉 **The pattern:** External system calls your webhook → You load state → Update and continue
...
```python title="src/recover_orders_step.py"
from datetime import datetime
    ...
    for order in orders:
        ...
        if order["status"] != "complete":
            ...
            if now - last_update > stuck_time:
                context.logger.warn("Found stuck order, retrying", {
                    "orderId": order["id"],
                    "currentStep": order["current_step"]
                })
...
- [Adapters →](/docs/development-guide/adapters)
- [Flows →](/docs/development-guide/flows)
- [Observability →](/docs/development-guide/observability)
