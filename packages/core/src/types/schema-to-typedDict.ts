...
class Parser {
  ...
  private pos(): number | 'EOF' {
    return this.peek()?.pos ?? 'EOF'
  }
  ...
  private parseRecordType(): TypeNode {
    ...
    if (key.value !== 'string') throw new SyntaxError(`Only Record<string, ...> supported at pos ${key.pos}`)
    this.expect('COMMA')
    const val = this.parseType()
    this.expect('GREATER_THAN')
...
function typeSignature(t: TypeNode): Sig {
  switch (t.node) {
    case 'Base':
      return ['base', t.kind]
    case 'Array':
      return ['array', typeSignature(t.item)]
    case 'MappingType':
      return ['mapping', typeSignature(t.value)]
    case 'Unknown':
      return ['unknown']
    case 'Enum':
      return ['enum', [...t.options].sort()]
    case 'Obj': {
...
const validIdentifier = (n: string) => /^[A-Za-z_]\w*$/.test(n) && !PYTHON_KEYWORDS.has(n)
...
function toPythonType(t: TypeNode): string {
  switch (t.node) {
    ...
    case 'Obj':
      ...
      return t.name
    case 'MappingType':
      return `Mapping[str, ${toPythonType(t.value)}]`
    case 'Enum':
      return `Literal[${t.options.map((o) => JSON.stringify(o)).join(', ')}]`
    case 'Union':
      return t.members.map(toPythonType).join(' | ')
    case 'Unknown':
      return 'Any'
  }
}
