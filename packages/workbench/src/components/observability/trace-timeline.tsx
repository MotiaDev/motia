...
export const TraceTimeline = ({ trace, group }: TraceTimelineProps) => {
            ...
            <div className="space-y-2">
              {details.emitOperations.map((op: EmitOperation) => (
                <div key={op.id} className="flex items-center justify-between text-xs bg-background rounded p-2">
                  ...
                  <div className="text-muted-foreground">{formatTimestamp(op.timestamp, baseTime)}</div>
                </div>
              ))}
            </div>
          </div>
                    ...
                    <Button variant="ghost" className="w-full p-0 h-auto">
                      <div className="flex items-center gap-4 w-full p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="w-32 text-sm font-medium truncate text-left">{step.name}</div>
                        </div>
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Badge className={getStatusColor(traceGroup.status)}>{traceGroup.status}</Badge>
        </div>
        ...
        {traceGroup.traces.map((trace, index) => (
          <Card key={trace.id} className="mb-4">
            ...
            <CardContent>{renderStepTimeline(trace.steps, trace.duration || 0, trace.id)}</CardContent>
          </Card>
        ))}
      </div>
    )
  ...
  if (group) {
    return <div className="h-full overflow-auto p-6">{renderTraceGroupTimeline(group)}</div>
  }
      ...
      <div className="h-full overflow-auto p-6">
        <div className="mb-6">
          ...
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            ...
            <span>
              {trace.metadata.completedSteps}/{trace.metadata.totalSteps} steps completed
            </span>
            <Badge variant="outline">{trace.entryPoint.type}</Badge>
        ...
        <Card>
          <CardContent className="p-6">{renderStepTimeline(trace.steps, trace.duration || 0, trace.id)}</CardContent>
        </Card>
