 ...
 */
import type { ApiResponse, ApiRouteHandler, CronHandler, EventHandler, MotiaStream } from 'motia'
...
declare module 'motia' {
  ...
  interface Handlers {
    'Test State With Python': EventHandler<
      unknown,
      { topic: 'test-state-check'; data: { key: string; expected?: unknown } }
    >
    ...
    OpenAiApi: ApiRouteHandler<
      ...
      {
        topic: 'openai-prompt'
        data: { message: string; assistantMessageId: string; threadId: string }
      }
    >
    CheckStateChange: EventHandler<{ key: string; expected: string }, never>
    SetStateChange: EventHandler<
      { message: string },
      { topic: 'check-state-change'; data: { key: string; expected: string } }
    >
    ApiTrigger: ApiRouteHandler<
      {
        pet: { name: string; photoUrl: string }
        foodOrder?: { id: string; quantity: number }
      },
      ApiResponse<200, { id: number; name: string; photoUrl: string }>,
      {
        topic: 'process-food-order'
        data: { email: string; quantity: number; petId: number }
      }
    >
    PeriodicJobHandled: EventHandler<{ message: string }, { topic: 'tested'; data: never }>
    HandlePeriodicJob: CronHandler<{
      topic: 'periodic-job-handled'
      data: { message: string }
    }>
    StateAuditJob: CronHandler<{
      topic: 'notification'
      data: {
        templateId: string
        email: string
    ...
    ProcessFoodOrder: EventHandler<
      { email: string; quantity: number; petId: number },
      {
        topic: 'notification'
        data: {
          templateId: string
    ...
    OpenAiApiPython: ApiRouteHandler<
      { message: string; threadId?: string },
      ApiResponse<200, { threadId: string }>,
      {
        topic: 'openai-prompt-python'
        data: { message: string; assistantMessageId: string; threadId: string }
      }
    >
    PythonStateAuditJob: CronHandler<{
    ...
    PythonProcessFoodOrder: EventHandler<
      { id: string; email: string; quantity: unknown; pet_id: unknown },
      {
        topic: 'python-notification'
        data: {
          template_id: string
    ...
    >
    PythonNotification: EventHandler<
      {
        template_id: string
        email: string
        template_data: Record<string, unknown>
      ...
      never
    >
    PythonApiTrigger: ApiRouteHandler<
      { pet: unknown; food_order?: unknown },
      ApiResponse<200, { id: unknown; name: string; photoUrl: string }>,
      {
        topic: 'python-process-food-order'
        data: { id: string; email: string; quantity: unknown; pet_id: unknown }
      }
    >
  }
}
